...
function getLatestValue(observations) {
  // Filter out missing values and get the latest
...
export default async function handler(req, res) {
  try {
    ...
    const gdpChange = gdpValue && gdpPrior ? ((gdpValue - gdpPrior) / gdpPrior) * 100 * 4 : null; // Annualized
    ...
    const fedFundsPrior = getPriorValue(fedFundsObs, 3); // Compare to 3 months ago
    ...
    // Match dates between 2Y and 10Y series
    ...
    const gdpHistory = gdpObs
      ...
      .map(o => ({
        ...
        value: parseFloat(o.value) / 1000 // Convert to trillions
      }))
      .reverse();
    ...
    for (let i = 0; i < Math.min(12, startsValid.length); i++) {
      ...
      // Find mortgage rate closest to this date
      const mortgage = mortgageValid.find(m => {
        const mDate = new Date(m.date);
        return Math.abs(mDate - startDate) < 45 * 24 * 60 * 60 * 1000; // Within 45 days
      });
      
      housingData.push({
        date: formatDate(startsValid[i].date),
        starts: parseFloat(startsValid[i].value) / 1000, // Convert to millions
    ...
    // Sahm Rule approximation (3-month avg unemployment vs 12-month low)
